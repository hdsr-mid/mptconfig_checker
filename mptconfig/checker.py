...
class MptConfigChecker:
    ...
    def run(self):
        ...
        # write new config csvs
        do_write_new_hoofdloc = False if self._hoofdloc_new is None else True
        writer = NewConfigCsvWriter(
            hoofdloc=self.hoofdloc,
            subloc=self.subloc,
            waterstandloc=self.waterstandloc,
            mpt_histtags=self.mpt_histtags,
            validation_csvs_new=self.validation_csvs_new,
            do_write_new_hoofdloc=do_write_new_hoofdloc,
        )
        writer.run()
